...
- (void)addDelegate:(id<PGMidiSourceDelegate>)delegate
{
    ...
    //printf("\ndelegate added: %d", [self.delegates count]);
}
...
- (void) midiRead:(const MIDIPacketList *)pktlist
{
    ...
    //printf("\nmidi read, delegate count %d", [delegates count]);
...
void PGMIDIVirtualDestinationReadProc(const MIDIPacketList *pktlist, void *readProcRefCon, void *srcConnRefCon)
{
    ...
    //printf("\nmidi source count %d", [[midi sources] count]);
    PGMidiSource *theVirtSource = midi.virtualDestinationSource;
    //printf("\ntheself exists? %ld", theself);
    [theVirtSource midiRead:pktlist];
}
...
- (id) init
{
    if ((self = [super init]))
    {
        ...
        OSStatus s = MIDIClientCreate((CFStringRef)@"MobMuPlat MIDI Client", PGMIDINotifyProc, (__bridge void*)self, &client);
        NSLogError(s, @"Create MIDI client");

        s = MIDIOutputPortCreate(client, (CFStringRef)@"MobMuPlat Output Port", &outputPort);
        NSLogError(s, @"Create output MIDI port");

        s = MIDIInputPortCreate(client, (CFStringRef)@"MobMuPlat Input Port", PGMIDIReadProc, arc_cast<void>(self), &inputPort);
