...
public class Widget extends RelativeLayout implements PdListener {
	...
	public Widget(Context context, float scale) {
		...
		//paint.setStrokeWidth(scale); nope looks bad...smaller val?
	}
	...
	public void drawLabel(Canvas canvas) {
		if (label != null) {
			...
			canvas.drawText(label, /*dRect.left +*/ labelpos[0], /*dRect.top +*/ labelpos[1] + labelsize * scale / 3, paint);
			//paint.setTextSize(fontsize); //set it back. move. NO everyone else should set it before use
		}
		paint.setColor(Color.BLACK);
	}
	
	public void replaceDollarZero(int dollarZero) {
		//DEI - make sure name is sanitized to be null if "-" or "empty"
	}
