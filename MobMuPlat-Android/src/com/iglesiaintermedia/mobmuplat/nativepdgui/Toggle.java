...
public class Toggle extends Widget {
	...
	public Toggle(Context context, String[] atomline, float scale) {
		...
		sendValueOnInit = Integer.parseInt(atomline[6]) > 0;  //DEI check that send val on init works.
		sendname = atomline[7]; //app.app.replaceDollarZero(atomline[7]); TODO handle dollar sign zero. necc? Never more than one instance...
		setReceiveName(atomline[8]);
		label = sanitizeLabel(atomline[9]);
		...
		setValue(Float.parseFloat(atomline[17])); //DEI check that send val on init works.
...
//		setupreceive();
	...
	protected void receiveBangFromSource(String source) {
		toggle();
		sendFloat(getValue()); // DEI change to sendValue() ?
	}
